...
export const ListUserPolls = () => {
    ...
    const {data:polls,isPending} = useQuery({
      ...
      queryFn: async () => await getUserPolls({data: {
        userId: session.user.id
      }})
    })
  
    if(isPending || !polls) {
      return <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
        {Array.from({ length: 6 }, (_, index) => index + 1).map((_)=>{
          return <Skeleton />
        })}
      </ul>
    }
    ...
    return (
      <>
      <h2 className="text-4xl font-sgc font-semibold text-primary tracking-wider pb-3">Mis encuestas</h2>
