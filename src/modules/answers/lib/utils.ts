			...
			case "geolocation": {
				if (q.isRequired) {
					shape[q.id] = z.object(
						...
						{ error: "La ubicación geográfica es obligatoria." },
					);
				} else {
					shape[q.id] = z
						.object({
						...
						.refine(
							(val) => {
								if (!val) return true;
								const hasLat =
									val.latitude !== undefined && val.latitude !== null;
								const hasLng =
									val.longitude !== undefined && val.longitude !== null;
								return (hasLat && hasLng) || (!hasLat && !hasLng);
							},
							{
								message:
									"Debes proporcionar tanto la latitud como la longitud, o dejar ambas vacías.",
							},
						);
				}
				break;
