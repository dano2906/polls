...
import { Map as MapComponent } from "@/ui/map";
...
export function ResponseRenderer({ question }: Props) {
	const { type, textResponse, selectedAnswers, questionText, order } = question;
	...
	// Aislamos el renderizado del cuerpo para mantener el contenedor principal limpio
	const renderResponseContent = () => {
		...
		switch (type) {
					...
					<div className="space-y-2">
						...
						<div className="grid gap-2">
							{orderedAnswers.map((ans, idx) => (
								<div
									key={ans.id}
									className="flex items-center gap-3 p-2.5 bg-background border rounded-lg text-sm font-medium shadow-xs"
									// Nota: Cambié shadow-sm por shadow-xs para homogeneizar con el nuevo estándar de Tailwind v4 si aplica, puedes revertirlo si usas v3.
								>
			...
			case "multiple_choice": {
				const selectedIds: string[] = selectedAnswers
					.filter((sa) => sa.wasSelected)
					.map((sa) => sa.id);
				const isQuiz = selectedAnswers.some(
						...
						<div className="grid gap-2">
							{distribution.map((ans, idx) => {
								...
								return (
									<div
										key={ans.id}
										className="relative flex items-center justify-between p-2 border rounded-lg bg-background shadow-xs overflow-hidden group font-sgc"
									>
										<div
										...
										<div className="relative z-10 flex items-center gap-2.5 w-full pr-4">
											{isWinner ? (
												<Trophy className="h-4 w-4 text-primary shrink-0 drop-shadow-xs" />
											) : (
										...
										<div className="relative z-10 flex items-center gap-3 shrink-0">
											...
											<Badge
												className={cn(
													"flex items-center justify-center rounded-xs tracking-wider text-xs font-bold min-w-10 shadow-xs",
													isWinner
			...
			case "geolocation": {
				const lat = selectedAnswers[0]?.lat;
				const lng = selectedAnswers[0]?.lng;

				// Validamos que las coordenadas existan y sean del tipo correcto
				const isValidCoords =
					typeof lat === "number" && typeof lng === "number";

				return (
					<div className="space-y-3 w-full">
						{/* Tarjeta de información con las coordenadas */}
							...
							<div className="flex flex-col gap-0.5">
								...
								<span className="text-sm font-semibold text-foreground tracking-wide font-mono">
									{isValidCoords
										...
										: "Coordenadas ilegibles"}
								</span>
							</div>
						</div>

						{/* Contenedor del mapa con desborde controlado */}
						{isValidCoords ? (
							<div className="w-full h-60 overflow-hidden border rounded-lg bg-muted/10 shadow-xs">
								<MapComponent center={[lat, lng]} zoom={12} />
							</div>
						) : (
	...
	// Estructura contenedora que agrupa la pregunta y su respuesta correspondiente
	return (
		<div className="flex flex-col gap-4 p-5 bg-background border rounded-xl shadow-xs">
			{/* Encabezado: Orden y Enunciado */}
			<div className="flex items-start gap-3">
				<span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-muted text-xs font-bold text-muted-foreground border border-muted-foreground/10 select-none">
					{order}
				</span>
				<h3 className="text-sm font-semibold tracking-tight text-foreground pt-0.5 leading-5">
					{questionText}
				</h3>
			</div>

			{/* Cuerpo: Espaciado hacia la derecha para mantener jerarquía visual con el número */}
			<div className="pl-9 w-full">{renderResponseContent()}</div>
		</div>
	);
}
