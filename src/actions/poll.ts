...
export const getUserPolls = createServerFn({method: 'GET'}).inputValidator((data: {userId:string})=>data).handler(async ({data}) => {
	try {
		...
		}).from(poll).where(eq(poll.userId,data.userId)).orderBy(asc(poll.startDate))
